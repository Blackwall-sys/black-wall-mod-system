// BWMS — `cw-utils` cauda (Vector3 helpers, 2026-07-15): declaração IDÊNTICA à fonte real
// (enablers/Codeware/scripts/Utils/Vector3.reds). 100% REDSCRIPT PURO — `Vector3` já é um tipo
// NATIVO do próprio motor (não forjado por nós); estes são só operadores de conveniência sobre
// campos já existentes (X/Y/Z). ZERO risco de crash (não registra native, não toca RTTI/validador
// de classe) — o único risco possível é erro de compilação, não de boot.
