...
fn main() {
    ...
    // argumentos na linha de comando final (achado em `tweakdb-tool` 2026-07-15, ao adicionar
    // um `[lib]` ao pacote — mesmo risco latente aqui se este crate algum dia ganhar um `[lib]`).
    // `-force_load` inclui TODOS os símbolos do `.a` incondicionalmente, sem depender de ordem.
